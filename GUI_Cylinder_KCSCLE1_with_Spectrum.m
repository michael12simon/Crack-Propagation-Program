function varargout = GUI_Cylinder_KCSCLE1_with_Spectrum(varargin)
% GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM MATLAB code for GUI_Cylinder_KCSCLE1_with_Spectrum.fig
%      GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM, by itself, creates a_input new GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM returns the handle to a_input new GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_CYLINDER_KCSCLE1_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Cylinder_KCSCLE1_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Cylinder_KCSCLE1_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Cylinder_KCSCLE1_with_Spectrum

% Last Modified by GUIDE v2.5 03-Oct-2019 13:51:38

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Cylinder_KCSCLE1_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Cylinder_KCSCLE1_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Cylinder_KCSCLE1_with_Spectrum is made visible.
function GUI_Cylinder_KCSCLE1_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Cylinder_KCSCLE1_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Cylinder_KCSCLE1_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Cylinder_KCSCLE1_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Cylinder_KCSCLE1_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
% Initial Condition
crack = 0; stage = 1; internal_pressure = 1;
pc_max = 0; pc_min = 0;
...
Ro = Ri + t;
wtb = waitbar(0,'KCSCLE1 calculation is running, please wait');
...
while KI_max_0 <= KIC && KI_max_90 <= KIC && KI_max <= KIC;
    ...
    if surface == true;
        ...
        [KI_max_0, KI_min_0, KI_max_90, KI_min_90]= KCSCLE1(a, c, t, Ri, Ro, P_max, P_min, beta_0, beta_90, inside);
