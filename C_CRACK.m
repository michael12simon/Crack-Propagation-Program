function varargout = C_CRACK(varargin)
% C_CRACK MATLAB code for C_CRACK.fig
%      C_CRACK, by itself, creates a new C_CRACK or raises the existing
%      singleton*.
%
%      H = C_CRACK returns the handle to a new C_CRACK or the handle to
%      the existing singleton*.
%
%      C_CRACK('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in C_CRACK.M with the given input arguments.
%
%      C_CRACK('Property','Value',...) creates a new C_CRACK or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before C_CRACK_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to C_CRACK_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help C_CRACK

% Last Modified by GUIDE v2.5 08-Oct-2019 11:01:46

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @C_CRACK_OpeningFcn, ...
                   'gui_OutputFcn',  @C_CRACK_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before C_CRACK is made visible.
function C_CRACK_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to C_CRACK (see VARARGIN)

% Choose default command line output for C_CRACK
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes C_CRACK wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = C_CRACK_OutputFcn(hObject, eventdata, handles) 
