function varargout = GUI_Plate_KPECL_with_Spectrum(varargin)
% GUI_PLATE_KPECL_WITH_SPECTRUM MATLAB code for GUI_Plate_KPECL_with_Spectrum.fig
%      GUI_PLATE_KPECL_WITH_SPECTRUM, by itself, creates a_input new GUI_PLATE_KPECL_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_PLATE_KPECL_WITH_SPECTRUM returns the handle to a_input new GUI_PLATE_KPECL_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_PLATE_KPECL_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_PLATE_KPECL_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_PLATE_KPECL_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_PLATE_KPECL_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Plate_KPECL_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Plate_KPECL_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Plate_KPECL_with_Spectrum

% Last Modified by GUIDE v2.5 26-Sep-2019 10:08:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Plate_KPECL_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Plate_KPECL_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Plate_KPECL_with_Spectrum is made visible.
function GUI_Plate_KPECL_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Plate_KPECL_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Plate_KPECL_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Plate_KPECL_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Plate_KPECL_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
d2 = t - d1;
wtb = waitbar(0,'KPECL calculation is running, please wait');
...
while KI_max <= KIC && KI_max_0 <= KIC;
    ...
    if d1 > d2;
        fprintf('d1 > d2, please check input value')
        break
    end
    ...
    if embedded == true;    
        ...
        a = (aA + aB)/2;
        alpha = 2*a/t;
        sigma_ref = (sigma_b_max + 3*sigma_m_max*alpha + ((sigma_b_max + 3*sigma_m_max*alpha)^2 + 9*sigma_m_max^2*((1 - alpha)^2 + 4*dA*alpha/t))^0.5)/(3*(1 - alpha)^2 + 4*dA*alpha/t);

        % Stress Intensity Calculation
        beta_A = beta0 + beta1*dA/t + beta2*(dA/t)^2 + beta3*(dA/t)^3 + beta4*(dA/t)^4 + beta5*(dA/t)^5;
        beta_B = beta0 + beta1*(dA+2*a)/t + beta2*((dA+2*a)/t)^2 + beta3*((dA+2*a)/t)^3 + beta4*((dA+2*a)/t)^4 + beta5*((dA+2*a)/t)^5;
        
        [KI_max_A, KI_min_A, KI_max_B, KI_min_B] = KPECL(a,t,d1,pc_max, pc_min, beta_A,beta_B,sigma0_max, sigma1_max, sigma2_max, sigma3_max, sigma4_max, sigma0_min, sigma1_min, sigma2_min, sigma3_min, sigma4_min);
        KI_max_0 = KI_max_A;
        KI_min_0 = KI_min_A;
        KI_max_90 = KI_max_B;
        KI_min_90 = KI_min_B;
    ...
    %SURFACE CRACK CALCULATION        
        ...
        % Stress Intensity Calculation
        beta = beta0 + beta1*a/t + beta2*(a/t)^2 + beta3*(a/t)^3 + beta4*(a/t)^4 + beta5*(a/t)^5;
