function varargout = GUI_Cylinder_KCSCLE2_with_Spectrum(varargin)
% GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM MATLAB code for GUI_Cylinder_KCSCLE2_with_Spectrum.fig
%      GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM, by itself, creates a_input new GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM returns the handle to a_input new GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_CYLINDER_KCSCLE2_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Cylinder_KCSCLE2_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Cylinder_KCSCLE2_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Cylinder_KCSCLE2_with_Spectrum

% Last Modified by GUIDE v2.5 03-Oct-2019 14:14:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Cylinder_KCSCLE2_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Cylinder_KCSCLE2_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Cylinder_KCSCLE2_with_Spectrum is made visible.
function GUI_Cylinder_KCSCLE2_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Cylinder_KCSCLE2_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Cylinder_KCSCLE2_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Cylinder_KCSCLE2_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Cylinder_KCSCLE2_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
Ri = str2double(get(handles.t_input,'String'))/1000;
...
% Initial Condition
crack = 0; internal_pressure = 0; % Using Membrane and Bending Stress Calc in KCTCL
...
Ro = Ri + t;
P_max = 0; P_min = 0; %Cond to avoid Internal Press only calc in KCTCL
wtb = waitbar(0,'KCSCLE2 calculation is running, please wait');
...
while KI_max_0 <= KIC && KI_max_90 <= KIC && KI_max <= KIC;
    ...
    if surface == true;
        if a/c < 0.03125 || a/c > 2;
            fprintf('a/c does not meet criteria 0.03125 <= a/c <= 2')
            f = msgbox('a/c does not meet criteria 0.03125 <= a/c <= 2','Completed');
            disp(f)
            break
        end
    end
    ...
    % Transition from Surface to Through Crack
    if a/t > 0.8;
        crack = 1;
        surface = false;
    end
    ...
    if surface == true;
        ...
        lamda = 1.818*c/(Ri*t)^0.5;
        Mt = ((1.02 + 0.4411*lamda^2 + 0.006124*lamda^4)/(1.0 + 0.02642*lamda^2 + 1.533*(10e-6)*lamda^4))^0.5;
        Ms = (1 - 0.85*a/t*(1/Mt))/(1 - 0.85*a/t);
        sigma_ref = (g*sigma_b_max+((g*sigma_b_max)^2 + 9*(Ms*sigma_m_max*(1 - alpha)^2)^2)^0.5)/(3*(1 - alpha)^2);
        ...
        [KI_max_0, KI_min_0, KI_max_90, KI_min_90]= KCSCLE2(a, c, t, Ri, beta_0, beta_90, inside, pc_max, pc_min, sigma0_max, sigma0_min, sigma1_max, sigma1_min, sigma2_max, sigma2_min, sigma3_max, sigma3_min, sigma4_max, sigma4_min);
    ...
    % THROUGH CRACK CALCULATION
        ...
        Mt = ((1.02 + 0.4411*lamda^2 + 0.006124*lamda^4)/(1.0 + 0.02642*lamda^2 + 1.533*(10e-6)*lamda^4))^0.5;
        beta = beta0;
        sigma_ref = (sigma_b_max+(sigma_b_max^2 + 9*(Mt*sigma_m_max)^2)^0.5)/3;

        % Stress Intensity Calculation
        [KI_max_out, KI_min_out, KI_max_in, KI_min_in]= KCTCL(c , t, Ri, Ro, internal_pressure,P_max, P_min, pc_max, pc_min, beta, sigma_m_max, sigma_m_min, sigma_b_max, sigma_b_min);
            
        if KI_max_out >= KI_max_in;
            KI_max = KI_max_out;
            KI_min = KI_min_out;
        else
            KI_max = KI_max_in;
        ...
        if hydrogen_timed == 1;
            da_timed = Hydrogen_Timed(crack, timestep, KI_max, KTH, KI_max_0, KI_max_90);
            c = c + da_timed;
        end
