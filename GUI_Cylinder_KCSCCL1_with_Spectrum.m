function varargout = GUI_Cylinder_KCSCCL1_with_Spectrum(varargin)
% GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM MATLAB code for GUI_Cylinder_KCSCCL1_with_Spectrum.fig
%      GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM, by itself, creates a_input new GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM returns the handle to a_input new GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_CYLINDER_KCSCCL1_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Cylinder_KCSCCL1_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Cylinder_KCSCCL1_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Cylinder_KCSCCL1_with_Spectrum

% Last Modified by GUIDE v2.5 03-Oct-2019 11:05:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Cylinder_KCSCCL1_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Cylinder_KCSCCL1_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Cylinder_KCSCCL1_with_Spectrum is made visible.
function GUI_Cylinder_KCSCCL1_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Cylinder_KCSCCL1_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Cylinder_KCSCCL1_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Cylinder_KCSCCL1_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Cylinder_KCSCCL1_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
%        str2double(get(hObject,'String')) returns contents of F_max as a_input double
...
%        str2double(get(hObject,'String')) returns contents of M_max_input as a_input double
...
%        str2double(get(hObject,'String')) returns contents of F_min_input as a_input double
...
%        str2double(get(hObject,'String')) returns contents of M_min_input as a_input double
...
Ro = Ri + t;
wtb = waitbar(0,'KCSCCL1 calculation is running, please wait');
...
sigma_m_max_0 = P_max*Ri^2/(Ro^2 - Ri^2) + F_max/(pi*(Ro^2 - Ri^2)) + 2*M_max*(Ro + Ri)/(pi*(Ro^4 - Ri^4));
sigma_m_max_reff_0 = P_max*Ri^2/(Ro^2 - Ri^2) + F_max/(pi*(Ro^2 - Ri^2));
sigma_bg_max_reff_0 = 2*M_max*Ro/(0.25*pi*(Ro^4 - Ri^4));
sigma_m_max = P_max*Ri^2/(Ro^2 - Ri^2) + F_max/(pi*(Ro^2 - Ri^2)) + 2*M_max*(Ro + Ri)/(pi*(Ro^4 - Ri^4));
sigma_m_max_reff = P_max*Ri^2/(Ro^2 - Ri^2) + F_max/(pi*(Ro^2 - Ri^2));
sigma_bg_max_reff = 2*M_max*Ro/(0.25*pi*(Ro^4 - Ri^4));
P_max_0 = P_max;
P_min_0 = P_min;
F_max_0 = F_max;
F_min_0 = F_min;
...
while KI_max <= KIC;
    ...
    if a/t > 0.8;
        fprintf('a/t > 0.8');
        f = msgbox('a/t exceeded 0.8','Completed');
        disp(f)
        break
    end
    ...
    if spectrum_load == 1;
        ...
        if (2*i - 1) >= (z + 1)*spectrum_rows;
            if spectrum(spectrum_rows,2) > spectrum(1,2);
                ...
                sigma_m_max_reff = sigma_m_max_reff_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);
                sigma_bg_max_reff = sigma_bg_max_reff_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);
                P_max = P_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
                P_min = P_min_0*spectrum(1,2)/spectrum(2,2);
                F_max = F_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
                F_min = F_min_0*spectrum(1,2)/spectrum(2,2);
                M_max = M_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
                M_min = M_min_0*spectrum(1,2)/spectrum(2,2);
            else
                i = i + 1;
        ...
        elseif i > 1 || (2*i) == (z + 1)*spectrum_rows;
            ...
            sigma_m_max_reff = sigma_m_max_reff_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            sigma_bg_max_reff = sigma_bg_max_reff_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            P_max = P_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            P_min = P_min_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);    
            F_max = F_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            F_min = F_min_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2); 
            M_max = M_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            M_min = M_min_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2); 
        end
    end
    
    % Calculation of Reference Stress
    if inside == 1;
        Nr = sigma_m_max_reff*(Ro^2 - Ri^2)/(Ro^2 - (Ri + a)^2);
        Mr = sigma_bg_max_reff*3*pi/16*((Ro^4 - Ri^4)/(Ro^4 - Ri*(Ri + a)^3));
    else
        Nr = sigma_m_max_reff*(Ro^2 - Ri^2)/((Ro - a)^2 - Ri^2);
        Mr = sigma_bg_max_reff*3*pi/16*((Ro^4 - Ri^4)/(Ro*(Ro - a)^3 - Ri^4));
    end
    
    sigma_ref = Mr/2 + (Nr^2 + Mr^2/4)^0.5;
    
    % Stress Intensity Calculation
    [KI_max, KI_min]= KCSCCL1(a, t, Ri, Ro, beta, inside, pc_max, pc_min, P_max, P_min, F_max, F_min, M_max, M_min);
