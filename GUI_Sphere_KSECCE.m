function varargout = GUI_Sphere_KSECCE(varargin)
% GUI_SPHERE_KSECCE MATLAB code for GUI_Sphere_KSECCE.fig
%      GUI_SPHERE_KSECCE, by itself, creates a_input new GUI_SPHERE_KSECCE or raises the existing
%      singleton*.
%
%      H = GUI_SPHERE_KSECCE returns the handle to a_input new GUI_SPHERE_KSECCE or the handle to
%      the existing singleton*.
%
%      GUI_SPHERE_KSECCE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_SPHERE_KSECCE.M with the given input arguments.
%
%      GUI_SPHERE_KSECCE('Property','Value',...) creates a_input new GUI_SPHERE_KSECCE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Sphere_KSECCE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Sphere_KSECCE_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Sphere_KSECCE

% Last Modified by GUIDE v2.5 04-Oct-2019 11:52:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Sphere_KSECCE_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Sphere_KSECCE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Sphere_KSECCE is made visible.
function GUI_Sphere_KSECCE_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Sphere_KSECCE (see VARARGIN)

% Choose default command line output for GUI_Sphere_KSECCE
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Sphere_KSECCE wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Sphere_KSECCE_OutputFcn(hObject, eventdata, handles) 
...
wtb = waitbar(0,'KSECCE calculation is running, please wait');
...
while KI_max_0 <= KIC &&  KI_max_90 <= KIC && KI_max <= KIC;
    ...
    if embedded == true;    
        ...
        [KI_max_90,KI_max_0,KI_min_90,KI_min_0]= KSECCE(a, c, d1, t, beta_0, beta_90, sigma0_max, sigma1_max, sigma2_max, sigma3_max, sigma4_max, sigma0_min, sigma1_min, sigma2_min, sigma3_min, sigma4_min);
    ...
    elseif surface == true;
        % Calculation of Reference Stress
        alpha = (a/t)/(1 + t/c);
        lamda = 1.818*c/sqrt(Ri*t);
        Mt = sqrt((1.0005 + 0.49001*lamda + 0.32409*lamda^2)/(1.0 + 0.50144*lamda - 0.011067*lamda^2));
        Ms = (1 - 0.85*(a/t)*(1/Mt))/(1 - 0.85*a/t); %API page 9C-5 Sec 9c.2.3.3.a
        ...
        % Stress Intensity Calculation
        [KI_max_0, KI_min_0, KI_max_90, KI_min_90] = KSSCCE2(a, c, t, Ri, pc_max, pc_min, beta_0, beta_90, sigma0_max, sigma1_max, sigma0_min, sigma1_min, sigma2_max, sigma3_max, sigma2_min, sigma3_min, sigma4_max, sigma4_min, inside);
        if KI_max_0 < 0 || KI_min_0 < 0 || KI_max_90 < 0 || KI_min_90 < 0;
    ...
    % THROUGH CRACK CALCULATION
        ...
        % Stress Intensity Calculation
        beta = beta0;
        [KI_max_in, KI_min_in, KI_max_out, KI_min_out] = KSTC(t, c, Ri, Ro, beta, internal_pressure, sigma_m_max, sigma_b_max, sigma_m_min, sigma_b_min, P_max, P_min, pc_max, pc_min);
