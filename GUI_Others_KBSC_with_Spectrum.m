function varargout = GUI_Others_KBSC_with_Spectrum(varargin)
% GUI_OTHERS_KBSC_WITH_SPECTRUM MATLAB code for GUI_Others_KBSC_with_Spectrum.fig
%      GUI_OTHERS_KBSC_WITH_SPECTRUM, by itself, creates a_input new GUI_OTHERS_KBSC_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_OTHERS_KBSC_WITH_SPECTRUM returns the handle to a_input new GUI_OTHERS_KBSC_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_OTHERS_KBSC_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_OTHERS_KBSC_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_OTHERS_KBSC_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_OTHERS_KBSC_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Others_KBSC_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Others_KBSC_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Others_KBSC_with_Spectrum

% Last Modified by GUIDE v2.5 07-Oct-2019 13:53:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Others_KBSC_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Others_KBSC_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Others_KBSC_with_Spectrum is made visible.
function GUI_Others_KBSC_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Others_KBSC_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Others_KBSC_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Others_KBSC_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Others_KBSC_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
%        str2double(get(hObject,'String')) returns contents of F_max_input as a_input double
...
function Rth_input_Callback(hObject, eventdata, handles)
% hObject    handle to Rth_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Rth_input as text
%        str2double(get(hObject,'String')) returns contents of Rth_input as a double


% --- Executes during object creation, after setting all properties.
function Rth_input_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Rth_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
Rth = str2double(get(handles.Rth_input,'String'))/1000;
...
F_bar_max = str2double(get(handles.F_max_input,'String'));
F_bar_min = str2double(get(handles.F_min_input,'String'));
M_bar_max = str2double(get(handles.M_max_input,'String'));
M_bar_min = str2double(get(handles.M_min_input,'String'));
...
wtb = waitbar(0,'KBSC calculation is running, please wait');
...
F_bar_max_0 = F_bar_max;
F_bar_min_0 = F_bar_min;
M_bar_max_0 = M_bar_max;
M_bar_min_0 = M_bar_min;
...
while KI_max <= KIC;
    ...
    zeta = a/(2*Rth);
    if zeta < 0.004 || zeta > 0.5;
        fprintf('zeta out of range 0.004 <= zeta >= 0.6')
        f = msgbox('zeta out of range 0.004 <= zeta >= 0.6','Completed');
        disp(f)
        break
    end
    ...
    if spectrum_load == 1;
        ...
        if (2*i - 1) >= (z + 1)*spectrum_rows;
            if spectrum(spectrum_rows,2) > spectrum(1,2);
                F_bar_max = F_bar_max_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);
                F_bar_min = F_bar_min_0*spectrum(1,2)/spectrum(2,2);
                M_bar_max = M_bar_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
                M_bar_min = M_bar_min_0*spectrum(1,2)/spectrum(2,2);
            else
                i = i + 1;
        ...
        elseif i > 1 || (2*i) == (z + 1)*spectrum_rows;
            F_bar_max = F_bar_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            F_bar_min = F_bar_min_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);
            M_bar_max = M_bar_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            M_bar_min = M_bar_min_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);           
        end
    end
    
    % Calculation of Reference Stress
    sigma_m_max = F_bar_max/(pi*Rth^2);
    sigma_m_min = F_bar_min/(pi*Rth^2);
    sigma_b_max = 4*M_bar_max/(pi*Rth^3);
    sigma_b_min = 4*M_bar_min/(pi*Rth^3);
    sigma_bg = 4*M_bar_max/(pi*Rth^3);
    b = asin((Rth - a)/Rth);
    w = 1.0002 - 3.9927*(a/(2*Rth))^1.5 + 5.8491*(a/(2*Rth))^2.5 - 2.855*(a/(2*Rth))^3;
    sigma_ref = pi*sigma_m_max/(pi/2 + (sin(2*b))/2 + b) + 3*pi*sigma_bg/(16*w);
        
    % Stress Intensity Calculation
    d1 = Rth - a;
    beta = beta0 + beta1*d1/Rth + beta2*(d1/Rth)^2 + beta3*(d1/Rth)^3 + beta4*(d1/Rth)^4 + beta5*(d1/Rth)^5;
    [KI_max, KI_min] = KBSC(a, Rth, sigma_m_max, sigma_m_min, sigma_b_max, sigma_b_min, beta);
