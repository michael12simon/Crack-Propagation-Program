function varargout = GUI_Others_KBSCL_with_Spectrum(varargin)
% GUI_OTHERS_KBSCL_WITH_SPECTRUM MATLAB code for GUI_Others_KBSCL_with_Spectrum.fig
%      GUI_OTHERS_KBSCL_WITH_SPECTRUM, by itself, creates a_input new GUI_OTHERS_KBSCL_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_OTHERS_KBSCL_WITH_SPECTRUM returns the handle to a_input new GUI_OTHERS_KBSCL_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_OTHERS_KBSCL_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_OTHERS_KBSCL_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_OTHERS_KBSCL_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_OTHERS_KBSCL_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Others_KBSCL_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Others_KBSCL_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Others_KBSCL_with_Spectrum

% Last Modified by GUIDE v2.5 07-Oct-2019 11:55:30

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Others_KBSCL_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Others_KBSCL_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Others_KBSCL_with_Spectrum is made visible.
function GUI_Others_KBSCL_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Others_KBSCL_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Others_KBSCL_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Others_KBSCL_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Others_KBSCL_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
wtb = waitbar(0,'KBSCL calculation is running, please wait');
...
while KI_max <= KIC;
    ...
    sigma_m_max = F_bar_max/(pi*(Ro - a)^2);
    sigma_m_min = F_bar_min/(pi*(Ro - a)^2);
    sigma_b_max = 4*M_bar_max/(pi*(Ro - a)^3);
    sigma_b_min = 4*M_bar_min/(pi*(Ro - a)^3);
    sigma_bg = 4*M_bar_max/(pi*Ro^3);
    Nr = sigma_m_max*Ro^2/((Ro - a)^2);
    Mr = sigma_bg*3*pi/16*(Ro^4/(Ro*(Ro - a)^3));
    ...
    [KI_max, KI_min] = KBSCL(a, Ro, sigma_m_max, sigma_m_min, sigma_b_max, sigma_b_min, beta);
