function varargout = GUI_Others_KRCSCCL1_with_Spectrum(varargin)
% GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM MATLAB code for GUI_Others_KRCSCCL1_with_Spectrum.fig
%      GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM, by itself, creates a_input new GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM returns the handle to a_input new GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_OTHERS_KRCSCCL1_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Others_KRCSCCL1_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Others_KRCSCCL1_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Others_KRCSCCL1_with_Spectrum

% Last Modified by GUIDE v2.5 07-Oct-2019 10:32:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Others_KRCSCCL1_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Others_KRCSCCL1_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Others_KRCSCCL1_with_Spectrum is made visible.
function GUI_Others_KRCSCCL1_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Others_KRCSCCL1_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Others_KRCSCCL1_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Others_KRCSCCL1_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Others_KRCSCCL1_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
function S_inside_input_Callback(hObject, eventdata, handles)
% hObject    handle to S_inside_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of S_inside_input as text
%        str2double(get(hObject,'String')) returns contents of S_inside_input as a double


% --- Executes during object creation, after setting all properties.
function S_inside_input_CreateFcn(hObject, eventdata, handles)
% hObject    handle to S_inside_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function S_outside_input_Callback(hObject, eventdata, handles)
% hObject    handle to S_outside_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of S_outside_input as text
%        str2double(get(hObject,'String')) returns contents of S_outside_input as a double


% --- Executes during object creation, after setting all properties.
function S_outside_input_CreateFcn(hObject, eventdata, handles)
% hObject    handle to S_outside_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
Ar = str2double(get(handles.Ar_input,'String'))/1000000;
...
S_inside = str2double(get(handles.S_inside_input,'String'));
S_outside = str2double(get(handles.S_outside_input,'String'));
...
Ro = Ri + t;
wtb = waitbar(0,'KRCSCCL1 calculation is running, please wait');
...
while KI_max <= KIC;
    ...
    if a/t < 0.2 || a/t > 0.8;
        fprintf('a/t does not meet 0.2 <= a/t <= 0.8')
        f = msgbox('a/t does not meet 0.2 <= a/t <= 0.8','Completed');
        disp(f)
        break
    end
    ...
    if spectrum_load == 1;
        ...
        if (2*i - 1) >= (z + 1)*spectrum_rows;
            if spectrum(spectrum_rows,2) > spectrum(1,2);
                S_inside = S_inside_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);
                S_outside = S_outside_0*spectrum((2*i - 1 - z*spectrum_rows),2)/spectrum(2,2);
        ...
        elseif i > 1 || (2*i) == (z + 1)*spectrum_rows;
            S_inside = S_inside_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            S_outside = S_outside_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
            P_max = P_max_0*spectrum((2*i - z*spectrum_rows),2)/spectrum(2,2);
    ...
    alpha = a/t;
    tau = t/Ro;
    Z = (1 - alpha*((2 - 2*tau + alpha*tau)/(2 - tau)))^-1;
    sigma_m_max = (S_inside + S_outside)/2;
    sigma_b_max = abs(S_inside - S_outside)/2;
    sigma_ref = (sigma_b_max + (sigma_m_max^2 + 9*(Z*sigma_m_max*(1 - alpha)^2)^2)^0.5)/(3*(1 - alpha)^2);
        
    % Stress Intensity Calculation
    d1 = t - a;
    beta = beta0 + beta1*d1/t + beta2*(d1/t)^2 + beta3*(d1/t)^3 + beta4*(d1/t)^4 + beta5*(d1/t)^5;
    [KI_max, KI_min] = KRCSCCL1(a, P_max, P_min, t, Ar, Ri, beta);
