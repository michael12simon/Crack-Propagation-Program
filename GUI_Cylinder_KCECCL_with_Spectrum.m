function varargout = GUI_Cylinder_KCECCL_with_Spectrum(varargin)
% GUI_CYLINDER_KCECCL_WITH_SPECTRUM MATLAB code for GUI_Cylinder_KCECCL_with_Spectrum.fig
%      GUI_CYLINDER_KCECCL_WITH_SPECTRUM, by itself, creates a_input new GUI_CYLINDER_KCECCL_WITH_SPECTRUM or raises the existing
%      singleton*.
%
%      H = GUI_CYLINDER_KCECCL_WITH_SPECTRUM returns the handle to a_input new GUI_CYLINDER_KCECCL_WITH_SPECTRUM or the handle to
%      the existing singleton*.
%
%      GUI_CYLINDER_KCECCL_WITH_SPECTRUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_CYLINDER_KCECCL_WITH_SPECTRUM.M with the given input arguments.
%
%      GUI_CYLINDER_KCECCL_WITH_SPECTRUM('Property','Value',...) creates a_input new GUI_CYLINDER_KCECCL_WITH_SPECTRUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Cylinder_KCECCL_with_Spectrum_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Cylinder_KCECCL_with_Spectrum_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Cylinder_KCECCL_with_Spectrum

% Last Modified by GUIDE v2.5 03-Oct-2019 16:58:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Cylinder_KCECCL_with_Spectrum_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Cylinder_KCECCL_with_Spectrum_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Cylinder_KCECCL_with_Spectrum is made visible.
function GUI_Cylinder_KCECCL_with_Spectrum_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a_input future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Cylinder_KCECCL_with_Spectrum (see VARARGIN)

% Choose default command line output for GUI_Cylinder_KCECCL_with_Spectrum
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_Cylinder_KCECCL_with_Spectrum wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Cylinder_KCECCL_with_Spectrum_OutputFcn(hObject, eventdata, handles) 
...
Ro = Ri + t;
wtb = waitbar(0,'KCECCL calculation is running, please wait');
...
while KI_max <= KIC && KI_max_0 <= KIC && KI_max_90 <= KIC;
    ...
    if d1 > d2;
        fprintf('d1 > d2, Check your input value')
        f = msgbox('d1 > d2, Check your input value','Error','error');
        disp(f)
        break
    end
    ...
    if embedded == true;
        crack_growth(i,3) = (aA+aB)*1000;
        crack_growth(i,4) = KI_max_0;
    ...
    if embedded == true;
        ...
        beta_A = beta0 + beta1*d/t + beta2*(d/t)^2 + beta3*(d/t)^3 + beta4*(d/t)^4 + beta5*(d/t)^5;
        beta_B = beta0 + beta1*(d+2*a)/t + beta2*((d+2*a)/t)^2 + beta3*((d+2*a)/t)^3 + beta4*((d+2*a)/t)^4 + beta5*((d+2*a)/t)^5;
        
        [KI_max_A, KI_min_A, KI_max_B, KI_min_B] = KCECCL(a,t,d1,pc_min, pc_max, beta_A, beta_B, sigma0_max, sigma1_max, sigma2_max, sigma3_max, sigma4_max, sigma0_min, sigma1_min, sigma2_min, sigma3_min, sigma4_min);
        KI_max_0 = KI_max_A;
        KI_min_0 = KI_min_A;
        KI_max_90 = KI_max_B;
        KI_min_90 = KI_min_B;
    ...
    %SURFACE CRACK CALCULATION
        ...
        % Stress Intensity Calculation
        d = t - a;
        beta = beta0 + beta1*d/t + beta2*(d/t)^2 + beta3*(d/t)^3 + beta4*(d/t)^4 + beta5*(d/t)^5;
        [KI_max, KI_min]= KCSCCL2(a, t, Ri, beta, inside, pc_max, pc_min, sigma0_max, sigma0_min, sigma1_max, sigma1_min, sigma2_max, sigma2_min, sigma3_max, sigma3_min, sigma4_max, sigma4_min);
...
ttl = {'Iteration', 'Time (Hour)', 'a (mm)', 'KI max A (MPa.m^0.5)', 'KI max B (MPa.m^0.5)', 'Stage'};
